...
public class ProxyOld implements Runnable {
    ...
    private static Logger logger = LoggerFactory.getLogger(ProxyOld.class);
    ...
    public ProxyOld(int port) {
        this.controlPortAddress = new InetSocketAddress(port);
    }

    public ProxyOld(String host, int port) {
    ...
    public static class ProxyConnection implements Connection {
        ...
        private ByteBuffer buffer = ByteBuffer.allocateDirect(2048); // Bigger then the biggest MTU
    ...
    protected static class ControlPort implements PortProcessor {
        ...
        private InetSocketAddress socketAddress;
        private ServerSocketChannel serverSocketChannel;
        private ProxyOld proxy;

        public ControlPort(ProxyOld proxy, Selector selector, InetSocketAddress socketAddress) {
            this.proxy = proxy;
            this.selector = selector;
            this.socketAddress = socketAddress;
        }
    ...
    protected static class ControlConnection implements Connection {
        ...
        public ControlConnection(ProxyOld proxy, SocketChannel channel) {
            this.proxy = proxy;
    ...
    public static void main(String[] args) throws Exception {
        ...
        ProxyOld proxy = new ProxyOld(8044);
        proxy.start();
